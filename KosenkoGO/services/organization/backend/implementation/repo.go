...
func (r *repository) CreateOrganization(org domain.Organization) (*domain.Organization, error) {
	sql := `
		INSERT INTO organizations (name, address, account_id, chief, financial_chief, okud, okpo)
	...
	var insertedId int
	if err := result.QueryRow(org.Name, org.Address, org.AccountId, org.Chief, org.FinancialChief, org.Okud, org.Okpo).Scan(&insertedId); err != nil {
		return nil, err
	}
...
func (r *repository) UpdateOrganization(org domain.Organization) (*domain.Organization, error) {
	sql := `
		UPDATE organizations SET name = $1, address = $2, account_id = $3, chief = $4, financial_chief = $5, okud = $6, okpo = $7 WHERE id = $8
	`
	rows, err := r.db.Query(sql, org.Name, org.Address, org.AccountId, org.Chief, org.FinancialChief, org.Okud, org.Okpo, org.Id)
	if err != nil {
		return nil, err
	}
	rows.Close()
